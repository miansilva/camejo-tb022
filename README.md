...
4. [`04-bash-y-regex.md`](./apuntes/04-bash-y-regex.md) - Historia y arquitectura de la Shell Unix, atajos de terminal, permisos prácticos y comprobación de recursos en scripts, Bash scripting, modo estricto (`set -euo pipefail`), expansiones de parámetros, redirecciones I/O, `[[ ... ]]` vs `[ ... ]`, `trap` y procesamiento de texto con Regex (`grep`, `sed`, `awk`).
